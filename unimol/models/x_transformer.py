...
class TransformerWrapper(nn.Module):
    ...
    def forward(
        self, x, t, mask=None, return_embeddings=False, return_mems=False, return_attn=False, mems=None, **kwargs
    ):
        #b, n, device, num_mem = *x.shape, x.device, self.num_memory_tokens
        b, n, dim, device = *x.shape, x.device
        #x = self.token_emb(x)
        #x += self.pos_emb(x)
        #x = self.emb_dropout(x)

        x = self.project_emb(x) + self.t_embedder(t)
