...
#save_dir=./saveOptimalDual # replace to your save path saveOptimalDualConsis
save_dir=./saveOptimalDualConsis # replace to your save path saveOptimalDualConsis
...
python -m torch.distributed.launch --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $(which unicore-train) $data_path --user-dir ./unimol --train-subset train --valid-subset valid \
       ...
       --find-unused-parameters --task unimol \
       --task-sub None \
       --training ae_only --loss unimol_MAE2 --arch ${arch} --wandb disabled \
       ...
       --save-interval-updates 5000 --validate-interval-updates 5000 --keep-interval-updates 50 --keep-best-checkpoints 3 --no-epoch-checkpoints  \
