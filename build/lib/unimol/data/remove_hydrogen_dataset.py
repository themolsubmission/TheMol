...
class RemoveHydrogenDataset(BaseWrapperDataset):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        # Filter out molecules with too many atoms (>= 128)
        # Original code (would raise error later in tokenize_dataset):
        # (no filtering)
        # Modified: Truncate molecules that are too large
        MAX_ATOMS = 128
        self.total_count += 1
        if len(atoms) >= MAX_ATOMS:
            self.truncated_count += 1
            atoms = atoms[:MAX_ATOMS - 1]
            coordinates = coordinates[:MAX_ATOMS - 1]
    ...
    def print_truncation_stats(self, split_name=""):
        """Print statistics about truncated samples"""
        if self.total_count > 0:
            percentage = (self.truncated_count / self.total_count) * 100
            print(f"[{split_name}] Truncation Statistics:")
            print(f"  Total samples processed: {self.total_count}")
            print(f"  Samples truncated (>= 128 atoms): {self.truncated_count}")
            print(f"  Truncation rate: {percentage:.2f}%")
        else:
            print(f"[{split_name}] No samples processed yet.")
