...
class DiT(nn.Module):
    ...
    def initialize_weights(self):
        ...
        # Initialize output layers with proper scale for flow matching:
        nn.init.constant_(self.final_layer.adaLN_modulation[-1].weight, 0)
        nn.init.constant_(self.final_layer.adaLN_modulation[-1].bias, 0)
        nn.init.normal_(self.final_layer.linear.weight, std=0.1)  # Increased from 0.02
        nn.init.constant_(self.final_layer.linear.bias, 0)

    def forward(self, x, t, mask, x_sc=None, spacegroup=None, dataset_idx=None):
        """Forward pass of DiT.
        ...
        tmp_cumsum = torch.cumsum(mask, dim=-1, dtype=torch.int64)
        tmp_cumsum[:,0] = tmp_cumsum.max(1).values
        token_index = tmp_cumsum - 1
