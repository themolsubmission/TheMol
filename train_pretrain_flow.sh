...
python -m torch.distributed.launch --nproc_per_node=$n_gpu --master_port=$MASTER_PORT $(which unicore-train) $data_path --user-dir ./unimol --train-subset train --valid-subset valid \
       ...
       --task-sub generative \
       --training flow_only --loss unimol_MAE2 --arch ${arch} --wandb disabled \
       ...
       --save-interval-updates 5000 --validate-interval-updates 2500 --keep-interval-updates 50 --keep-best-checkpoints 3 --no-epoch-checkpoints  \
